...
.post(async(req,res)=>{
   ...
   if(signoutDate.getHours()<=4){
    ...
    const array = await attendance_model.find({ id:id }).sort({ signInDate : -1})
    console.log("hoba")
    ...
    for (let index = 0; index < todayArray.length; index++) {
        ...
        if(todayArray[index].signOutDate==null){
            console.log('omaaar')
            await attendance_model.findOneAndUpdate({id:id,_id:todayArray[index]._id},{signOutDate:signoutDate})
            const x = (await attendance_model.findOne({id:id,_id:todayArray[index]._id}))
            const signoutTime=x.signOutDate.getTime()
            var a = x.signInDate.getTime()
            var Hours=(signoutTime-a)/(60000*60)
            await attendance_model.findOneAndUpdate({id:id,_id:todayArray[index]._id},{$set:{hours:Hours}})
